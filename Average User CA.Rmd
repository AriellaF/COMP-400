...
```{r}
<<<<<<< HEAD
cleaned[names(select(cleaned, ends_with(".F")))][is.na(cleaned[names(select(cleaned, ends_with(".F")))])] 
=======
cleaned[names(select(cleaned, ends_with(".F")))][is.na(cleaned[names(select(cleaned, ends_with(".F")))])]<-0
>>>>>>> parent of dc2cefc (final report)
...
aggregate(select(cleaned, ends_with(".F")), by=list(cleaned$Age,cleaned$Meal.Prepper,cleaned$Dietary.Restriction,cleaned$Home.Cook.Rate), FUN=mean)
```
