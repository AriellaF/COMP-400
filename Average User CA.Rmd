...
#isolate users who responded to AB Question
no.na<-my.data
for (f in names(select(my.data.factored, starts_with("Ing")))){
  # no.na<-drop_NA(my.data, f)
 # no.na<-my.data[!is.na(my.data[f])]
 print(str(my.data.factored[f]))
...
for (i in names(select(cleaned, starts_with("Ing.")))){
  p<-ggplot(cleaned,aes( fill=Dietary.Restriction))+
    geom_bar(aes_string(x=i))+
    facet_grid(~Meal.Prepper)+
...
for (i in names(select(note.takers, starts_with("Ing.")))){
  p<-ggplot(note.takers,aes_string( fill=i))+
    ...
    # facet_grid(~Dietary.Restriction+Meal.Prepper)+
    theme(axis.text.x = element_text(angle = 90))
