...
```{r}
# cleaned%>% 
#   group_by(Age,Meal.Prepper,Dietary.Restriction,Home.Cook.Rate)%>%
#  summarise(sum)
...
```{r}
# grouped.df<-cleaned%>%group_by(Meal.Prepper,Dietary.Restri ction,Home.Cook.Rate)
# grouped.df
for (f in names(select(cleaned, ends_with(".F")))){
  p<-ggplot(cleaned, aes(x=Meal.Prepper, fill=Home.Cook.Rate))+
...
for (i in names(select(cleaned, starts_with("Ing.")))){
  p<-ggplot(cleaned,aes( fill=Home.Cook.Rate))+
